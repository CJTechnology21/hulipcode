...
const fetchProducts = async (category, subcategory) => {
  try {
    const readableSub = subcategory.replace(/-/g, " ");
    const res = await axios.get(
      `http://localhost:5000/api/products?category=${category}&subCategorySlug=${readableSub}`
    );
  } catch (err) {
                  ...
                  >
                    ...
                    <ul className="space-y-1 text-gray-800 text-sm">
                      {items.map((item) => {
                        ...
                        const slug = name.toLowerCase().replace(/\s+/g, "-"); // ✅ define slug here
                        return (
                          <li key={name}>
